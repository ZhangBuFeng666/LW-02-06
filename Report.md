...
| 姓名 | 学号 | 分工与产出 | 贡献占比 |
|------|------|-----------|---------|
| 章步峰 | 23301114 | 项目架构、Mock 后端、接口设计、ServiceContext、路由、主链路联调、最终自检打包 |
| 徐滔 | 23301110 | 首页、分类页、商品详情页，完成搜索、轮播推荐、热门商品和商品浏览体验 |
| 薛探昕 | 23301141 | 用户登录/注册、购物车、创建订单、支付、订单列表、订单详情、我的页面 | 
| 徐健智 | 23301139 | 后台登录、角色权限、商品管理增删改查、上下架、订单查看发货和前后台联动演示 | 
| 张浩然 | 23301115 | 前端页面布局修改，接口调整修复、功能测试、Report、PPT、答辩脚本和演示流程控制 |
...
- [x] **分页/无限滚动**：在前台分类商品列表页面（CategoryPage）实现前端分页，每页限制展示 6 件商品，当筛选条件变化时自动重置页码并渲染精美的分页交互组件。
- [x] **支付模拟优化**：支付页包含二维码式支付区和倒计时。
- [x] **响应式布局**：使用 CSS Grid 和媒体查询适配移动端。
- [x] **性能优化**：实现路由组件懒加载（React.lazy）、Rollup 核心库打包分割（manualChunks）、不常变分类数据内存缓存，并全面应用 Unsplash 高清大图云端压缩尺寸与图片原生懒加载。
- [x] **单元测试**：引入 Vitest 与 jsdom 测试环境，对 src/services 服务层进行 100% 全量覆盖测试，用例通过率 100%，整体覆盖率达到 98% 以上。
